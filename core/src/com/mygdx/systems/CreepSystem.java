...
public class CreepSystem extends IteratingSystem implements RayCastCallback{
    ...
    public CreepSystem(PlayScreen playScreen) {
    ...
    protected void processEntity(Entity entity, float deltaTime) {
        ...
        playScreen.getWorld().rayCast(this,
                 from.x, from.y,
                from.x + dir.x,
                from.y + dir.y);
    ...
    public float reportRayFixture(Fixture fixture, Vector2 point, Vector2 normal, float fraction) {
        ...
        if(userData != null){
            ...
            if(
                    userData.getType() == Constants.PLAYER ||
                    userData.getType() == Constants.POWER_UP ||
                    userData.getType() == Constants.CREEP
                    ) {
                return -1;
            }
        }
    ...
    private void pickRandomDirection(){
        ...
        Vector2 newPosition = playScreen.world_to_maze_coords(physicComponent.getBody().getPosition());

        newPosition = playScreen.maze_to_world_coords(newPosition.x, newPosition.y);

         physicComponent.getBody().setTransform(newPosition.x / Constants.PPM,
        newPosition.y / Constants.PPM, 0);
    ...
    private Array<Integer> getDirections(){
        ...
        Vector2 mazePosition = playScreen.world_to_maze_coords(physicComponent.getBody().getPosition());
